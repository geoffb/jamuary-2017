...
const CAMERA_PLANE_LENGTH = 0.666;
const TEXTURE_SIZE = 32 * 4;

let FirstPerson = module.exports = function () {
  Node.call(this);
...
object.extend(FirstPerson, Node);

FirstPerson.prototype.setCamera = function (posX, posY, dirX, dirY) {
  let cam = this.camera;
...
FirstPerson.prototype._render = function (context) {
...
FirstPerson.prototype._drawEntities = function (context) {
  var entities = this.level.entities;
  var len = entities.length;
  var order = new Array(len);
  var distance = new Array(len);
  var i, entity, transform;

  var texture = assets.getTexture(this.textureKey);

  var cam = this.camera;
  var camX = cam.position.x;
  ...
  for (i = 0; i < len; ++i) {
    ...
    order[i] = i;
    distance[i] = ((camX - transform.position.x) * (camX - transform.position.x) + (camY - transform.position.y) * (camY - transform.position.y));
  }
  ...
  for (i = 0; i < len; ++i) {
    ...
    transform = entity.getComponent("transform");
    var entityX = transform.position.x - camX;
    var entityY = transform.position.y - camY;
    ...
    if (drawStartY > this.height || drawEndY < 0) { continue; }
    ...
    if (drawStartX > this.width || drawEndX < 0) { continue; }

    var sprite = entity.getComponent("sprite");

    // loop through every vertical stripe of the sprite on screen
    for (var x = drawStartX; x < drawEndX; ++x) {
      if (transformY > 0 && x > 0 && x < this.width && transformY < this._zbuffer[x]) {
        ...
        texX += sprite.texture * TEXTURE_SIZE;
